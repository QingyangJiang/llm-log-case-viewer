...
- 宠物装备与技能：摸摸、提交标注、发现 Badcase 均可能触发装备掉落；每次从 3 件同稀有度候选中选择 1 件，界面会提供仓库数量、合成缺口、主题收藏、套装档位与同部位穿戴对比，随机词条领取后才鉴定。图鉴包含 300 件、5 个穿戴部位和 5 档稀有度。3 件同名装备可概率合成至 Lv.10，并生成可暴击、可重复叠加的随机效果词条；还可消耗同名材料洗练词条，或将任意装备分解为零件后进行二合一随机熔铸。每件装备都有固定效果，同主题 2/3/5 件可激活套装共鸣
