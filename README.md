...
- 宠物装备与技能：摸摸、提交标注、发现 Badcase 均可能掉落装备；图鉴包含 300 件、5 个穿戴部位和 5 档稀有度。重复装备可自动升至 Lv.5，每件装备都有实际效果，同主题 2/3/5 件可激活套装共鸣。进化可觉醒 8 种技能，最多启用 3 个并实际影响掉率或进化概率
...
装备图鉴由 12 个主题、5 个部位和 5 档稀有度组合生成，共 300 件。摸摸基础掉率为 2.5%，每个候选首次提交为 12%，首次标记 Badcase 可额外再抽一次；技能和已穿戴装备会继续加成。重复获得同一装备会自动升级，最高 Lv.5，旧账号已有数量会直接映射为对应等级。不同部位分别提升全局装备掉率、摸摸/标注/Badcase 专属掉率或单抽进化概率；同主题装备达到 2/3/5 件时，依次获得全局掉率、稀有权重和进化概率套装加成。每次进化成功还会随机觉醒或升级一个技能，最高 Lv.5，最多同时启用 3 个。管理员可在“进化抽奖”页选择其他用户、填写数量并再次输入自己的管理员登录密码，安全发送 1–50 张进化券；每次发放都会留下服务端审计记录。
