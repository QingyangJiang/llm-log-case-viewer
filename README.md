...
- 宠物装备与技能：摸摸、提交标注、发现 Badcase 均可能掉落装备；图鉴包含 300 件、5 个穿戴部位和 5 档稀有度。进化可觉醒 8 种技能，最多启用 3 个并实际影响掉率或进化概率
...
装备图鉴由 12 个主题、5 个部位和 5 档稀有度组合生成，共 300 件。摸摸、提交标注和首次标记 Badcase 都有随机掉落机会；重复装备会累计数量，获得后可在工作室直接穿戴。每次进化成功还会随机觉醒或升级一个技能，最高 Lv.5，最多同时启用 3 个。管理员可在“进化抽奖”页选择其他用户、填写数量并再次输入自己的管理员登录密码，安全发送 1–50 张进化券；每次发放都会留下服务端审计记录。
