...
function PetCreatureVisual({ profile, accessory, showEquipment = true, showFashion = true }: { profile: PetProfile; accessory?: string; showEquipment?: boolean; showFashion?: boolean }) {
  ...
  return <span className={`pet-creature evolution-${path || "base"} evolution-stage-${profile.evolution_stage} evolution-variant-${profile.evolution_variant}`} aria-hidden="true">
    ...
    {fashionItems.map((item) => <span className={`pet-fashion pet-fashion-${item.slot} fashion-theme-${item.theme}`} style={{ "--fashion-primary": item.primary, "--fashion-secondary": item.secondary } as CSSProperties} title={item.name} key={item.slot}>{item.symbol}</span>)}
