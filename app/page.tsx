...
type PetEquipmentSort = "theme_slot" | "slot_theme" | "power" | "level" | "rarity" | "count" | PetEquipmentEffectKey;
...
const PET_WHEEL_REWARDS: PetWheelReward[] = [
  { id: "ticket_1", label: "进化券 ×1", short_label: "1张进化券", icon: "↟", probability: 70, tone: "lime" },
  { id: "ticket_2", label: "进化券 ×2", short_label: "2张进化券", icon: "↟", probability: 10, tone: "aqua" },
...
const PET_LEVELS = [
  ...
  { level: 4, title: "Badcase 侦探", unlock: "草莓甜心 · 薰衣草" },
  { level: 6, title: "质量守门员", unlock: "云朵梦游 · 珊瑚色" },
  { level: 8, title: "评测专家", unlock: "海盐航行 · 王冠" },
  { level: 10, title: "首席标注官", unlock: "森林茶会 · 星夜色" },
  { level: 15, title: "资深裁决师", unlock: "侦探事务所 · 翡翠毛色" },
  { level: 20, title: "传奇质检师", unlock: "霓虹未来 · 落日橘" },
  { level: 30, title: "评测领航员", unlock: "极光雪国 · 绮丽洋红" },
  { level: 40, title: "质量宗师", unlock: "赤焰华服 · 红宝石" },
  { level: 50, title: "Case Lens 守护者", unlock: "宇宙歌剧 · 满级荣誉" },
];
...
const PET_EQUIPMENT_EFFECTS: Record<PetEquipmentSlot, { key: Exclude<PetEquipmentEffectKey, "rarity_boost">; label: string }> = {
...
const PET_EQUIPMENT_MAX_LEVEL = 10;
const PET_EQUIPMENT_MAX_AFFIXES = 24;
...
const PET_EQUIPMENT_EFFECT_SORTS: { id: PetEquipmentEffectKey; label: string }[] = [
  { id: "pet_drop_bonus", label: "摸摸掉率加成" },
  { id: "evolution_bonus", label: "进化概率加成" },
  { id: "annotation_drop_bonus", label: "标注掉率加成" },
  { id: "badcase_drop_bonus", label: "Badcase 掉率加成" },
  { id: "all_drop_bonus", label: "全局掉率加成" },
  { id: "rarity_boost", label: "稀有装备权重" },
];

function petEquipmentEffectValue(item: PetEquipment, key: PetEquipmentEffectKey): number {
  const includesGlobalDrop = key === "pet_drop_bonus" || key === "annotation_drop_bonus" || key === "badcase_drop_bonus";
  const matches = (effect: PetEquipmentEffectKey) => effect === key || (includesGlobalDrop && effect === "all_drop_bonus");
  return (matches(item.effect_key) ? item.effect_value : 0)
    + item.affixes.reduce((total, affix) => total + (matches(affix.key) ? affix.value : 0), 0);
}
...
const PET_AUTO_EQUIP_MODES: { id: PetAutoEquipMode; icon: string; label: string; description: string }[] = [
  { id: "combat", icon: "⚔", label: "家园战力", description: "优先总战力，并计算已激活套装层数" },
  { id: "evolution", icon: "↟", label: "单抽进化", description: "最大化背饰、词条与套装的进化加成" },
  { id: "annotation", icon: "✓", label: "提交标注", description: "最大化所有掉率与标注专属掉率" },
  { id: "pet", icon: "♡", label: "摸摸寻宝", description: "最大化所有掉率与摸摸专属掉率" },
  { id: "badcase", icon: "!", label: "Badcase", description: "最大化所有掉率与 Badcase 专属掉率" },
  { id: "rarity", icon: "✦", label: "稀有收藏", description: "优先稀有装备权重，再比较战力" },
];
...
const PET_SKILL_DEFINITIONS: Omit<PetSkill, "level" | "active">[] = [
  ...
  { id: "collector", name: "图鉴学者", icon: "▦", description: "三选一装备的稀有权重 +1/级" }, { id: "steady_heart", name: "稳定之心", icon: "♥", description: "连续失败的保底增幅 +1%/级" },
];

function petEquipmentWithProgress(item: Pick<PetEquipment, "id" | "name" | "slot" | "slot_name" | "symbol" | "rarity" | "theme">, count: number, progress?: Partial<Pick<PetEquipment, "level" | "affixes" | "synthesis_failures">>): PetEquipment {
  const normalizedCount = Math.max(1, Math.floor(count || 1));
  const storedLevel = Number(progress?.level);
  const level = Math.max(1, Math.min(PET_EQUIPMENT_MAX_LEVEL, Number.isFinite(storedLevel) && storedLevel > 0 ? Math.floor(storedLevel) : Math.min(5, normalizedCount)));
  const affixes = Array.isArray(progress?.affixes) ? progress.affixes.filter((affix): affix is PetEquipmentAffix => isObject(affix) && typeof affix.key === "string" && affix.key in PET_EQUIPMENT_RANDOM_AFFIXES).slice(0, PET_EQUIPMENT_MAX_AFFIXES).map((affix, index) => ({ id: typeof affix.id === "string" ? affix.id : `affix-${index + 1}`, key: affix.key, label: PET_EQUIPMENT_RANDOM_AFFIXES[affix.key], value: Math.max(1, Math.min(99, Math.floor(Number(affix.value) || 1))), critical: Boolean(affix.critical) })) : [];
  const synthesisFailures = Math.max(0, Math.min(20, Math.floor(Number(progress?.synthesis_failures) || 0)));
  ...
  const effectValue = effect.key === "all_drop_bonus" ? Math.ceil(power / 2)
    : effect.key === "annotation_drop_bonus" ? Math.ceil(power * 0.75)
      : effect.key === "evolution_bonus" ? Math.ceil(power / 3)
        : power;
  const synthesisSuccessRate = level >= PET_EQUIPMENT_MAX_LEVEL ? 0 : Math.min(95, (PET_EQUIPMENT_SYNTHESIS_RATES[level] ?? 0) + Math.min(30, synthesisFailures * 5));
  return { ...item, count: normalizedCount, level, power: power + affixes.length, effect_key: effect.key, effect_label: effect.label, effect_value: effectValue, affixes, synthesis_failures: synthesisFailures, synthesis_success_rate: synthesisSuccessRate };
}
...
function petEquipmentState(inventory: PetEquipment[], equipped: Partial<Record<PetEquipmentSlot, string>>) {
  ...
  Object.entries(equipped).forEach(([slot, itemId]) => {
    ...
    item.affixes.forEach((affix) => { stats[affix.key] += affix.value; });
...
function petAutoLoadout(profile: PetProfile, mode: PetAutoEquipMode) {
  const slotEntries = Object.keys(PET_EQUIPMENT_SLOTS) as PetEquipmentSlot[];
  const isBetter = (left: readonly number[], right: readonly number[]) => left.some((value, index) => value > right[index] && left.slice(0, index).every((earlier, earlierIndex) => earlier === right[earlierIndex]));
  const itemValue = (item: PetEquipment) => {
    const values = { ...DEFAULT_EQUIPMENT_STATS };
    values.total_power = item.power;
    values[item.effect_key] += item.effect_value;
    item.affixes.forEach((affix) => { values[affix.key] += affix.value; });
    if (mode === "combat") return item.power;
    if (mode === "evolution") return values.evolution_bonus;
    if (mode === "annotation") return values.all_drop_bonus + values.annotation_drop_bonus;
    if (mode === "pet") return values.all_drop_bonus + values.pet_drop_bonus;
    if (mode === "badcase") return values.all_drop_bonus + values.badcase_drop_bonus;
    return values.rarity_boost;
  };
  const candidates = slotEntries.map((slot) => {
    const bestByTheme = new Map<string, PetEquipment>();
    profile.inventory.filter((item) => item.slot === slot).forEach((item) => {
      const current = bestByTheme.get(item.theme);
      if (!current || isBetter([itemValue(item), item.power, item.level], [itemValue(current), current.power, current.level])) bestByTheme.set(item.theme, item);
    });
    return [null, ...bestByTheme.values()] as (PetEquipment | null)[];
  });
  const loadoutScore = (items: (PetEquipment | null)[]) => {
    const equipped = Object.fromEntries(items.filter((item): item is PetEquipment => Boolean(item)).map((item) => [item.slot, item.id])) as Partial<Record<PetEquipmentSlot, string>>;
    const stats = { ...DEFAULT_EQUIPMENT_STATS };
    const themeCounts = new Map<string, number>();
    items.forEach((item) => {
      if (!item) return;
    ...
    let activeTiers = 0;
    themeCounts.forEach((pieces, theme) => PET_EQUIPMENT_SET_EFFECTS[theme].tiers.forEach((tier) => {
      if (pieces < tier.pieces) return;
      stats[tier.key] += tier.value;
      activeTiers += 1;
    }));
    const primary = mode === "combat" ? stats.total_power * 6 + activeTiers * 15
      : mode === "evolution" ? stats.evolution_bonus
        : mode === "annotation" ? stats.all_drop_bonus + stats.annotation_drop_bonus
          : mode === "pet" ? stats.all_drop_bonus + stats.pet_drop_bonus
            : mode === "badcase" ? stats.all_drop_bonus + stats.badcase_drop_bonus
              : stats.rarity_boost;
    return { equipped, score: [primary, stats.total_power, activeTiers] as const };
  };
  let best = loadoutScore([]);
  const visit = (slotIndex: number, selected: (PetEquipment | null)[]) => {
    if (slotIndex === candidates.length) {
      const result = loadoutScore(selected);
      if (isBetter(result.score, best.score)) best = result;
      return;
    }
    candidates[slotIndex].forEach((item) => visit(slotIndex + 1, [...selected, item]));
  };
  visit(0, []);
  return best;
}
...
function petEquipmentDropMessage(drop: PetDropEvent, source: "摸摸" | "标注") {
  ...
  return `${source}掉落同名材料「${drop.name}」，现有 ×${drop.count}`;
}
...
function normalizedPetProfile(value: Partial<PetProfile> | null | undefined): PetProfile {
  ...
  const inventory = Array.isArray(value?.inventory) ? value.inventory.filter((item): item is PetEquipment => isObject(item) && typeof item.id === "string").flatMap((item) => {
    ...
    return catalogItem ? [petEquipmentWithProgress(catalogItem, Number(item.count) || 1, item)] : [];
  ...
  const fashion = isObject(value?.fashion) ? Object.fromEntries(Object.entries(value.fashion).filter(([slot, itemId]) => {
    const item = PET_FASHION_CATALOG.find((candidate) => candidate.id === itemId);
    return slot in PET_FASHION_SLOTS && item?.slot === slot && item.level <= level;
  })) as Partial<Record<PetFashionSlot, string>> : {};
  const wardrobePresets = Array.isArray(value?.wardrobe_presets) ? value.wardrobe_presets.filter((preset): preset is PetWardrobePreset => isObject(preset) && typeof preset.id === "string").map((preset) => ({
    id: preset.id.slice(0, 120),
    name: typeof preset.name === "string" && preset.name.trim() ? preset.name.trim().slice(0, 30) : "未命名搭配",
    color: PET_COLORS.some((item) => item.id === preset.color) ? preset.color : "lime",
    accessory: PET_ACCESSORIES.some((item) => item.id === preset.accessory) ? preset.accessory : "none",
    fashion: isObject(preset.fashion) ? Object.fromEntries(Object.entries(preset.fashion).filter(([slot, itemId]) => {
      const item = PET_FASHION_CATALOG.find((candidate) => candidate.id === itemId);
      return slot in PET_FASHION_SLOTS && item?.slot === slot;
    })) as Partial<Record<PetFashionSlot, string>> : {},
    fashion_saved: typeof preset.fashion_saved === "boolean" ? preset.fashion_saved : isObject(preset.fashion),
    equipped: isObject(preset.equipped) ? Object.fromEntries(Object.entries(preset.equipped).filter(([slot, itemId]) => slot in PET_EQUIPMENT_SLOTS && typeof itemId === "string" && PET_EQUIPMENT_CATALOG.some((item) => item.id === itemId && item.slot === slot))) as Partial<Record<PetEquipmentSlot, string>> : {},
  ...
  const pendingDrops = Array.isArray(value?.pending_drops) ? value.pending_drops.filter((pending): pending is PetPendingDrop => isObject(pending) && typeof pending.token === "string" && Array.isArray(pending.choices)).flatMap((pending) => {
    const choices = pending.choices.flatMap((choice) => {
      ...
      const catalogItem = PET_EQUIPMENT_CATALOG.find((candidate) => candidate.id === choice.id);
      if (!catalogItem) return [];
      const base = petEquipmentWithProgress(catalogItem, 1);
      const hiddenAffix = isObject(choice.hidden_affix) ? petEquipmentWithProgress(catalogItem, 1, { affixes: [choice.hidden_affix as PetEquipmentAffix] }).affixes[0] : undefined;
      const equippedSameSlot = isObject(choice.equipped_same_slot) && typeof choice.equipped_same_slot.id === "string" ? {
        id: choice.equipped_same_slot.id,
        name: String(choice.equipped_same_slot.name ?? "当前装备"),
        rarity: (String(choice.equipped_same_slot.rarity ?? "common") as PetRarity),
  ...
  return {
    ...
    fashion,
    fashion_catalog_size: PET_FASHION_CATALOG.length,
    wardrobe_presets: wardrobePresets,
    skills,
...
function evolveLocalPet(profile: PetProfile, spend: 1 | 5 | 10, targetPath: PetEvolutionPath = "") {
  ...
  let targetFailuresAfter = profile.targeted_evolution_failures;
  let targetAfter = profile.targeted_evolution_target;
  let blessingsAfter = profile.targeted_evolution_blessings;
  let wheelCompensation = 0;
  if (success) {
    if (routeReset) {
    ...
    for (let offset = 0; offset < stageGain; offset += 1) {
      ...
      const traitPool = pools.traits[isIllustratedRoute(path) ? routeMilestoneIndex(nextStage + 1) : Math.min(Math.floor(nextStage / 2), pools.traits.length - 1)];
      let trait = traitPool[petRandomInt(traitPool.length)];
      if (isIllustratedRoute(path)) {
        if (nextStage + 1 > 15) trait = `${trait} · 进化${nextStage + 1}阶`;
      } else if (nextStage >= pools.traits.length * 2) trait = `${trait} · 星环${nextStage - pools.traits.length * 2 + 1}`;
  ...
  return {
    ...
    profile: normalizedPetProfile({
      ...
      targeted_evolution_blessings: blessingsAfter,
...
function randomLocalEquipmentAffix(item: PetEquipment, level: number, index: number): PetEquipmentAffix {
  const keys = Object.keys(PET_EQUIPMENT_RANDOM_AFFIXES) as PetEquipmentEffectKey[];
  const key = keys[petRandomInt(keys.length)];
  const ceiling = Math.max(1, 1 + Math.floor((PET_EQUIPMENT_RARITY_POWER[item.rarity] - 1) / 2) + Math.floor(Math.max(0, level - 1) / 3));
  const critical = petRandomInt(100) < 18;
  return { id: `affix-${Date.now()}-${petRandomInt(1_000_000_000)}-${index}`, key, label: PET_EQUIPMENT_RANDOM_AFFIXES[key], value: (petRandomInt(ceiling) + 1) * (critical ? 2 : 1), critical };
}

function localDropChoiceContext(profile: PetProfile, item: PetEquipment, hiddenAffix?: PetEquipmentAffix): PetDropChoice {
  const owned = profile.inventory.find((candidate) => candidate.id === item.id);
  const equippedId = profile.equipped[item.slot];
  const equippedSameSlot = profile.inventory.find((candidate) => candidate.id === equippedId);
  const themeOwnedCount = profile.inventory.filter((candidate) => candidate.theme === item.theme).length;
...
function claimLocalPetDrop(profile: PetProfile, token: string, itemId: string) {
  const pending = profile.pending_drops.find((entry) => entry.token === token);
  const choice = pending?.choices.find((entry) => entry.id === itemId);
  const catalogItem = PET_EQUIPMENT_CATALOG.find((entry) => entry.id === itemId);
  if (!pending || !choice || !catalogItem || !choice.hidden_affix) throw new Error("这次掉落已领取或已失效");
  const current = profile.inventory.find((owned) => owned.id === itemId);
  const affixAdded = (current?.affixes.length ?? 0) < PET_EQUIPMENT_MAX_AFFIXES;
  const affixes = affixAdded ? [...(current?.affixes ?? []), choice.hidden_affix] : current?.affixes ?? [];
  const claimed = petEquipmentWithProgress(catalogItem, (current?.count ?? 0) + 1, { level: current?.level ?? 1, affixes, synthesis_failures: current?.synthesis_failures ?? 0 });
  const inventory = current ? profile.inventory.map((owned) => owned.id === itemId ? claimed : owned) : [claimed, ...profile.inventory];
  const remaining = profile.pending_drops.filter((entry) => entry.token !== token).map((entry) => ({ ...entry, choices: entry.choices.map((entryChoice) => {
    const candidate = PET_EQUIPMENT_CATALOG.find((item) => item.id === entryChoice.id)!;
    return localDropChoiceContext({ ...profile, inventory }, candidate, entryChoice.hidden_affix);
  }) }));
  const drop: PetDropEvent = { ...claimed, reason: pending.reason, duplicate: Boolean(current), identified_affix: choice.hidden_affix, affix_added: affixAdded, at: new Date().toISOString() };
...
function synthesizeLocalPetEquipment(profile: PetProfile, itemId: string): PetSynthesisResult {
  ...
  const success = petRandomInt(100) < successRate;
  const gainedAffixes: PetEquipmentAffix[] = [];
  let nextItem: PetEquipment;
  if (success) {
    const nextLevel = item.level + 1;
    gainedAffixes.push(randomLocalEquipmentAffix(item, nextLevel, item.affixes.length));
    if (item.affixes.length + gainedAffixes.length < PET_EQUIPMENT_MAX_AFFIXES && petRandomInt(100) < 15) gainedAffixes.push(randomLocalEquipmentAffix(item, nextLevel, item.affixes.length + 1));
    nextItem = petEquipmentWithProgress(item, item.count - 2, { level: nextLevel, affixes: [...item.affixes, ...gainedAffixes], synthesis_failures: 0 });
  } else {
...
function reforgeLocalPetEquipment(profile: PetProfile, itemId: string) {
  const item = profile.inventory.find((owned) => owned.id === itemId);
  if (!item || item.count < 2) throw new Error("洗词条需要额外消耗 1 件同名装备");
  let affixCount = Math.max(1, item.affixes.length);
  if (affixCount < PET_EQUIPMENT_MAX_AFFIXES && petRandomInt(100) < 15) affixCount += 1;
  const affixes = Array.from({ length: affixCount }, (_, index) => randomLocalEquipmentAffix(item, item.level, index));
  const nextItem = petEquipmentWithProgress(item, item.count - 1, { ...item, affixes });
  return { profile: normalizedPetProfile({ ...profile, inventory: profile.inventory.map((owned) => owned.id === itemId ? nextItem : owned) }), affixes };
}

function dismantleLocalPetEquipment(profile: PetProfile, itemId: string) {
  const item = profile.inventory.find((owned) => owned.id === itemId);
  if (!item) throw new Error("仓库中没有这件装备");
  if (profile.equipped[item.slot] === item.id && item.count === 1) throw new Error("请先卸下这件装备再分解");
  const inventory = item.count > 1
    ? profile.inventory.map((owned) => owned.id === itemId ? petEquipmentWithProgress(owned, owned.count - 1, owned) : owned)
    : profile.inventory.filter((owned) => owned.id !== itemId);
  return { profile: normalizedPetProfile({ ...profile, inventory, equipment_parts: profile.equipment_parts + 1 }), dismantled: item, equipment_parts: profile.equipment_parts + 1 };
}

function chooseLocalPetRarity(profile: PetProfile) {
  const magnet = activePetSkillLevel(profile, "star_magnet") + activePetSkillLevel(profile, "collector") + profile.equipment_stats.rarity_boost;
  ...
  for (const [candidate, weight] of weights) { if (draw < weight) return candidate; draw -= weight; }
  return "common" as PetRarity;
}

function forgeLocalPetEquipment(profile: PetProfile): PetForgeResult {
  if (profile.equipment_parts < 2) throw new Error("随机熔铸需要 2 枚装备零件");
  const rarity = chooseLocalPetRarity(profile);
  const pool = PET_EQUIPMENT_CATALOG.filter((item) => item.rarity === rarity);
  const catalogItem = pool[petRandomInt(pool.length)];
  const current = profile.inventory.find((owned) => owned.id === catalogItem.id);
  const currentLevel = current?.level ?? 1;
  const levelUp = currentLevel < PET_EQUIPMENT_MAX_LEVEL && petRandomInt(100) < 1;
  const nextLevel = currentLevel + (levelUp ? 1 : 0);
  const affix = randomLocalEquipmentAffix(catalogItem, nextLevel, current?.affixes.length ?? 0);
  const affixAdded = (current?.affixes.length ?? 0) < PET_EQUIPMENT_MAX_AFFIXES;
  const nextItem = petEquipmentWithProgress(catalogItem, (current?.count ?? 0) + 1, {
    level: nextLevel,
    affixes: affixAdded ? [...(current?.affixes ?? []), affix] : current?.affixes ?? [],
    synthesis_failures: current?.synthesis_failures ?? 0,
  });
  const inventory = current ? profile.inventory.map((owned) => owned.id === current.id ? nextItem : owned) : [nextItem, ...profile.inventory];
...
function rollLocalPetDrop(profile: PetProfile, reason: Exclude<PetDropReason, "battle" | "wheel">) {
  ...
  if (petRandomInt(10000) >= Math.min(7500, chance)) return { profile, drop: null as PetDropEvent | null, pending: false };
  const rarity = chooseLocalPetRarity(profile);
  const pool = PET_EQUIPMENT_CATALOG.filter((item) => item.rarity === rarity);
  const choices: PetDropChoice[] = [];
  for (let index = 0; index < 3; index += 1) {
    const selectedIndex = petRandomInt(pool.length);
    const item = pool.splice(selectedIndex, 1)[0];
    const current = profile.inventory.find((owned) => owned.id === item.id);
    choices.push(localDropChoiceContext(profile, item, randomLocalEquipmentAffix(item, current?.level ?? 1, index)));
  }
  const pending: PetPendingDrop = { token: `local-${Date.now()}-${petRandomInt(1_000_000_000)}`, reason, at: new Date().toISOString(), choices };
  return { profile: normalizedPetProfile({ ...profile, pending_drops: [...profile.pending_drops, pending].slice(-PET_MAX_PENDING_DROPS) }), drop: null as PetDropEvent | null, pending: true };
}
...
function metricLatestAnnotations(records: CaseAnnotation[]) {
  const latest = new Map<string, { record: CaseAnnotation; rank: [number, number, number] }>();
  records.forEach((record, position) => {
    const key = `${record.annotator.id}\u0000${record.candidate_id}`;
    const rank: [number, number, number] = [Number(record.revision) || 0, Date.parse(record.updated_at) || 0, position];
    const previous = latest.get(key);
    if (!previous || rank[0] > previous.rank[0] || (rank[0] === previous.rank[0] && (rank[1] > previous.rank[1] || (rank[1] === previous.rank[1] && rank[2] >= previous.rank[2])))) latest.set(key, { record, rank });
  });
...
function buildMetricScope(items: { item: LogCase; index: number }[], records: Record<string, CaseAnnotation[]>, models: string[], dimensionKey: string, annotator?: { id: string; name: string }): MetricScope {
  ...
  items.forEach(({ item, index }) => {
    ...
    metricLatestAnnotations(records[caseAnnotationKey(item, index)] ?? []).forEach((record) => {
...
function buildLocalMetrics(items: LogCase[], records: Record<string, CaseAnnotation[]>, dimensionKey?: string, caseType: MetricCaseType = "all"): MetricsData {
  ...
  scopedItems.forEach(({ item, index }) => metricLatestAnnotations(records[caseAnnotationKey(item, index)] ?? []).forEach((record) => {
    ...
    <div className="pet-home-resident-stage">
      ...
      <div className="pet-home-resident-overview">
        ...
        <div className="pet-home-loadout">
          ...
          <div><small>装备</small><span>{resident.equipped_items.length ? resident.equipped_items.map((item) => <i className={`rarity-${item.rarity}`} title={`${item.name} · Lv.${item.level}`} key={item.slot}><PetEquipmentArt slot={item.slot} /></i>) : <em>暂无</em>}</span></div>
        </div>
      </div>
    </div>
    <div className="pet-home-resident-footer">
      ...
      <footer><div>{resident.active_sets.slice(0, 2).map((set) => <span key={set.theme}>{set.name} · {set.pieces}件</span>)}{!resident.active_sets.length ? <span>套装尚未激活</span> : null}</div><div>{resident.active_skills.map((skill) => <i title={`${skill.name} Lv.${skill.level}`} key={skill.id}>{skill.icon}<b>{skill.level}</b></i>)}{!resident.active_skills.length ? <small>暂无启用技能</small> : null}</div></footer>
    </div>
  </article>;
}
...
function PetHomestead({ open, user, data, busy, battleBusy, error, result, onRefresh, onBattle, onClose, onOpenTeam, onFinishResult }: {
  ...
  return <section className="pet-home-page" aria-label="宠物家园">
      ...
      {user && data ? <>
        <section className="pet-home-command">
          <div className="pet-home-command-scene" style={{ "--pet-color": PET_COLORS.find((item) => item.id === data.me.color)?.value ?? PET_COLORS[0].value } as CSSProperties}>
            ...
            {meProfile ? <div className="pet-home-main-pet"><i className="pet-home-pedestal" /><PetCreatureVisual profile={meProfile} accessory={myAccessory} showEquipment={false} /><span>{data.me.pet_name}</span></div> : null}
          </div>
    ...
    {result ? <div className={`pet-battle-result-backdrop ${result.outcome}${result.reward_pending ? " has-reward" : ""}`} role="presentation"><section className="pet-battle-result" role="dialog" aria-modal="true" aria-label="宠物战斗结果">
      ...
      <div className="pet-battle-verdict"><span>{result.reward_pending ? result.won ? "VICTORY REWARD" : "LUCKY DROP · 10%" : result.outcome === "draw" ? "DRAW" : "KEEP TRAINING"}</span><h3>{result.reward_pending ? result.won ? "发现一组装备战利品" : "虽败犹荣，触发意外装备掉落" : result.outcome === "draw" ? "战力相同，本次没有装备掉落" : "对手战力更高，本次没有装备掉落"}</h3><p>{result.reward_pending ? "三个候选的随机词条仍是隐藏状态，打开战利品后结合仓库与套装进度选择一件。" : "每日机会已经消耗。提升等级、继续进化或强化装备，可以提高下一次获胜概率。"}</p></div>
      <button type="button" onClick={() => onFinishResult(Boolean(result.reward_pending))}>{result.reward_pending ? "打开装备三选一" : "返回家园"}</button>
    </section></div> : null}
  </section>;
}
...
function PetDropChoiceModal({ pending, queueSize, busy, onClaim }: { pending?: PetPendingDrop; queueSize: number; busy: boolean; onClaim: (token: string, itemId: string) => void }) {
  ...
  return <div className="pet-drop-backdrop" role="presentation">
    <section className="pet-drop-modal" role="dialog" aria-modal="true" aria-label="装备掉落三选一">
      <header><div><span>EQUIPMENT DROP · {source.toUpperCase()}</span><h2>从三件装备中带走一件</h2><p>稀有度相同、随机词条隐藏；先根据库存、合成进度和套装收益做决定。</p></div><b>{queueSize > 1 ? `待领取 ${queueSize} 组` : "3 选 1"}</b></header>
      <div className="pet-drop-choice-grid">{pending.choices.map((choice, index) => <article className={`rarity-${choice.rarity}`} key={choice.id}>
        <div className="pet-drop-card-number"><span>候选 {String(index + 1).padStart(2, "0")}</span><b>{recommendation(choice)}</b></div>
        <div className="pet-drop-choice-title"><i><PetEquipmentArt slot={choice.slot} /></i><div><em>{PET_RARITY_LABELS[choice.rarity]} · {choice.slot_name}</em><strong>{choice.name}</strong><small>{choice.theme}主题 · 固定：{choice.effect_label} +{choice.effect_value}%</small></div></div>
        <div className="pet-drop-hidden-affix"><b>???</b><span>随机词条未鉴定<small>类型、数值及是否暴击，领取后揭晓</small></span></div>
        <div className="pet-drop-set-progress"><div><span>若装备后的套装进度</span><b>{choice.theme_pieces_if_equipped} / 5</b></div><i>{[1, 2, 3, 4, 5].map((piece) => <em className={`${piece <= choice.theme_pieces_if_equipped ? "filled" : ""} ${[2, 3, 5].includes(piece) ? "milestone" : ""}`} key={piece}>{[2, 3, 5].includes(piece) ? piece : ""}</em>)}</i></div>
        <dl className="pet-drop-warehouse">
      ...
      <footer><span>未选择会保存在账号中，刷新页面也不会丢失。</span><small>领取一件后，其余两件自动消失。</small><button type="button" onClick={() => setDismissedToken(pending.token)}>稍后选择</button></footer>
    </section>
  </div>;
}

function PetDropRevealModal({ result, queueSize, onClose }: { result: PetDropReveal | null; queueSize: number; onClose: () => void }) {
  if (!result) return null;
  const { drop, identified_affix: affix } = result;
  const value = `${affix.value}${affix.key === "rarity_boost" ? "" : "%"}`;
  return <div className="pet-drop-backdrop pet-reveal-backdrop" role="presentation">
    <section className={`pet-drop-reveal rarity-${drop.rarity}`} role="dialog" aria-modal="true" aria-label="装备词条鉴定结果">
      <header><span>IDENTIFICATION COMPLETE</span><b>{PET_RARITY_LABELS[drop.rarity]}装备</b></header>
      <div className="pet-reveal-item"><i><PetEquipmentArt slot={drop.slot} /></i><div><small>{drop.theme}主题 · {drop.slot_name}</small><h2>{drop.name}</h2><p>固定效果 · {drop.effect_label} +{drop.effect_value}%</p></div></div>
      <div className={`pet-reveal-affix ${affix.critical ? "critical" : ""}`}><span>{affix.critical ? "CRITICAL AFFIX" : "RANDOM AFFIX"}</span><strong>{affix.critical ? "暴击 · " : ""}{affix.label} +{value}</strong><small>{result.affix_added ? "词条已刻录到装备，后续可通过合成或洗练继续构筑。" : "原装备已有 24 条词条，本次仍获得同名材料，但词条未刻录。"}</small></div>
      <div className="pet-reveal-result"><span>{drop.duplicate ? "同名材料 +1" : "图鉴新收录"}</span><b>Lv.{drop.level} · 库存 ×{drop.count}</b></div>
      <button type="button" onClick={onClose}>{queueSize ? `继续领取下一组 · 还剩 ${queueSize}` : "收下装备"}</button>
    </section>
  </div>;
}
...
function PetForgeRevealModal({ result, onClose }: { result: PetForgeResult | null; onClose: () => void }) {
  ...
  const value = `${affix.value}${affix.key === "rarity_boost" ? "" : "%"}`;
  return <div className="pet-drop-backdrop pet-reveal-backdrop pet-forge-reveal-backdrop" role="presentation">
    <section className={`pet-drop-reveal pet-forge-reveal rarity-${item.rarity}`} role="dialog" aria-modal="true" aria-label="随机熔铸结果">
      <header><span>RANDOM FORGE · COMPLETE</span><b>已消耗 2 枚零件</b></header>
      <div className="pet-forge-reveal-mark"><i>⌘</i><span>熔铸完成</span><b>{result.level_up ? "1% 幸运升级触发" : "装备与词条已入库"}</b></div>
      <div className="pet-reveal-item"><i><PetEquipmentArt slot={item.slot} /></i><div><small>{PET_RARITY_LABELS[item.rarity]} · {item.theme}套装 · {item.slot_name}</small><h2>{item.name}</h2><p>固定效果 · {item.effect_label} +{item.effect_value}%</p></div></div>
      <div className={`pet-reveal-affix ${affix.critical ? "critical" : ""}`}><span>{affix.critical ? "CRITICAL AFFIX" : "IDENTIFIED AFFIX"}</span><strong>{affix.critical ? "暴击 · " : ""}{affix.label} +{value}</strong><small>{result.affix_added ? "随机词条已刻录到装备。" : "装备已有 24 条词条，本次仍增加同名库存，但词条未刻录。"}</small></div>
      <div className="pet-reveal-result"><span>{result.level_up ? "幸运强化 +1" : "随机熔铸奖励"}</span><b>Lv.{item.level} · 库存 ×{item.count}</b></div>
      <button type="button" onClick={onClose}>收下装备</button>
    </section>
  </div>;
}

function PetEquipmentGuide({ open, profile, onClose }: { open: boolean; profile: PetProfile; onClose: () => void }) {
  if (!open) return null;
  const slotEntries = Object.entries(PET_EQUIPMENT_SLOTS) as [PetEquipmentSlot, { label: string; symbol: string }][];
  const themeProgress = PET_EQUIPMENT_THEMES.map((theme) => {
    const items = profile.inventory.filter((item) => item.theme === theme);
    return { theme, items, copies: items.reduce((sum, item) => sum + item.count, 0), slots: new Set(items.map((item) => item.slot)).size };
  }).sort((a, b) => b.slots - a.slots || b.items.length - a.items.length || PET_EQUIPMENT_THEMES.indexOf(a.theme) - PET_EQUIPMENT_THEMES.indexOf(b.theme));
  const leadingTheme = themeProgress[0];
  const missingSlots = leadingTheme ? slotEntries.filter(([slot]) => !leadingTheme.items.some((item) => item.slot === slot)).map(([, info]) => info.label) : [];
  const equippedCount = Object.values(profile.equipped).filter(Boolean).length;
  const criticalAffixes = profile.inventory.reduce((sum, item) => sum + item.affixes.filter((affix) => affix.critical).length, 0);
  const guideRoutes = [
    { code: "A", title: "高频标注", themes: "蜂蜜 / 机械 / 霓虹", copy: "优先提交标注掉率与全局掉率，适合日常大量提交。" },
    { code: "B", title: "稀有收藏", themes: "星尘 / 琥珀 / 像素", copy: "先堆稀有权重，再用全局掉率扩大高品质装备产出。" },
    { code: "C", title: "进化冲刺", themes: "森林 / 月影 / 雷云", copy: "背饰固定效果配合套装进化加成，适合持续单抽强化。" },
    { code: "D", title: "Badcase 狩猎", themes: "海盐 / 纸片 / 月影", copy: "面饰和异常专属套装联动，放大 Badcase 的额外掉落机会。" },
  ];
  return <section className="pet-equipment-guide-page" aria-label="装备攻略">
    <header className="pet-guide-head"><div><span>EQUIPMENT FIELD MANUAL · 300 ITEMS</span><h2>装备构筑攻略</h2><p>从三选一、套装收集到合成洗练，把每一次掉落变成可规划的成长。</p></div><button type="button" onClick={onClose}>返回装备库</button></header>
    <div className="pet-guide-scroll">
      <section className="pet-guide-hero">
        <div className="pet-guide-hero-copy"><span>YOUR LOADOUT</span><h3>{profile.name}的装备路线</h3><p>{leadingTheme?.items.length ? `当前收集最完整的是「${leadingTheme.theme}」套装，已覆盖 ${leadingTheme.slots}/5 个部位、${leadingTheme.items.length}/25 件图鉴。${missingSlots.length ? `下一步优先补 ${missingSlots.join("、")}。` : "五个部位均已覆盖，可以比较稀有度、等级和词条质量。"}` : "仓库还没有装备。先通过摸摸、提交标注或标记 Badcase 触发三选一掉落。"}</p><div><span><b>{profile.inventory.length}</b> / 300 图鉴</span><span><b>{equippedCount}</b> / 5 穿戴</span><span><b>{profile.equipment_parts}</b> 熔铸零件</span><span><b>{criticalAffixes}</b> 暴击词条</span></div></div>
        <div className="pet-guide-cycle"><i>掉落</i><b>选择</b><i>套装</i><b>强化</b><i>构筑</i><span>先补空位，再追套装，最后精炼词条</span></div>
      </section>

      <section className="pet-guide-section"><header><span>01 · DROP DECISION</span><h3>三选一怎么选</h3><p>候选词条不可见，决定应优先基于确定信息。</p></header><div className="pet-guide-priority">{[
        ["1", "补齐空部位", "没有穿戴时，任何新部位都会立刻提供固定效果与战力。"],
        ["2", "跨过套装档位", "能让当前主题达到 2 / 3 / 5 件的候选，收益通常最确定。"],
        ["3", "准备同名合成", "领取后库存达到 3 件，就能尝试强化且保留主装备。"],
        ["4", "再比较稀有度", "同一轮候选稀有度相同；跨轮规划时，高稀有度底力更高。"],
      ].map(([step, title, copy]) => <article key={step}><b>{step}</b><div><strong>{title}</strong><p>{copy}</p></div></article>)}</div></section>

      <section className="pet-guide-section"><header><span>02 · BUILD ROUTES</span><h3>按目标选择套装</h3><p>12 套效果不同，先决定主要玩法，再追 2 / 3 / 5 件里程碑。</p></header><div className="pet-guide-route-grid">{guideRoutes.map((route) => <article key={route.code}><i>{route.code}</i><span><small>{route.title}</small><strong>{route.themes}</strong><p>{route.copy}</p></span></article>)}</div></section>

      <section className="pet-guide-section"><header><span>03 · SLOT MAP</span><h3>五个部位的固定职责</h3><p>固定效果只由部位决定，稀有度与等级提高数值。</p></header><div className="pet-guide-slot-map">{slotEntries.map(([slot, info], index) => <article key={slot}><i><PetEquipmentArt slot={slot} /></i><span><strong>{info.label}</strong><small>{PET_EQUIPMENT_EFFECTS[slot].label}</small></span><b>部位 {String(index + 1).padStart(2, "0")}</b></article>)}</div></section>

      <section className="pet-guide-section pet-guide-crafting"><header><span>04 · CRAFTING</span><h3>强化、洗练与熔铸</h3><p>强化追等级，洗练追词条，熔铸用来回收闲置装备。</p></header><div className="pet-guide-craft-grid"><article><span>同名强化</span><strong>3 件 → 保留 1 件主装备</strong><p>消耗 2 件材料，成功升 1 级并生成词条；失败不降级，且下次保底 +5%，最多 +30%。</p><div>{Object.entries(PET_EQUIPMENT_SYNTHESIS_RATES).map(([level, rate]) => <i key={level}><small>Lv.{level}</small><b>{rate}%</b></i>)}</div></article><article><span>词条洗练</span><strong>2 件 → 重置全部词条</strong><p>消耗 1 件同名材料，保留等级与合成保底。词条类型可重复，普通生成也可能额外暴击。</p><em>最多 24 条 · 暴击概率 18% · 可能额外生成 1 条</em></article><article><span>随机熔铸</span><strong>2 零件 → 1 件随机装备</strong><p>分解每件装备得到 1 枚零件。结果会直接鉴定词条，并有独立 1% 概率额外升 1 级。</p><em>不要分解最后一件已穿戴装备；稀有度、套装与部位均随机</em></article></div></section>

      <section className="pet-guide-section"><header><span>05 · COLLECTION MAP</span><h3>你的 12 套收集进度</h3><p>每套共 25 件图鉴：5 个部位 × 5 档稀有度。</p></header><div className="pet-guide-theme-progress">{themeProgress.map((entry) => <article key={entry.theme}><div><strong>{entry.theme}</strong><span>{PET_EQUIPMENT_SET_EFFECTS[entry.theme].name}</span><b>{entry.items.length} / 25</b></div><i><em style={{ width: `${entry.items.length / 25 * 100}%` }} /></i><small>{entry.slots} / 5 部位 · 共 {entry.copies} 件库存</small></article>)}</div></section>
    </div>
  </section>;
}
...
function CompanionPet({ visible, message, mood, completed, total, pulse, hasNext, profile, settingsOpen, draftName, busy, persistenceLabel, isAdmin, currentUserId, adminUsers, onPet, onEvolve, onUnlockPersona, onSwitchPersona, onEquip, onAutoEquip, onSynthesize, onReforge, onDismantle, onRandomForge, onOpenEquipmentGuide, onSaveWardrobe, onApplyWardrobe, onDeleteWardrobe, onToggleSkill, onGiftTickets, onSpinWheel, onCompleteWheelSpin, onGiftWheelChances, onNext, onHide, onShow, onToggleSettings, onDraftName, onSelectColor, onSelectAccessory, onSelectFashion, onSaveProfile }: {
  ...
  onRandomForge: () => void;
  onOpenEquipmentGuide: () => void;
  onSaveWardrobe: (name: string) => Promise<void>;
  onApplyWardrobe: (presetId: string) => void;
  ...
  onSelectFashion: (slot: PetFashionSlot, itemId: string | null) => void;
  onSaveProfile: () => void;
}) {
  ...
  const [equipmentSort, setEquipmentSort] = useState<PetEquipmentSort>("theme_slot");
  const [equipmentSetFocus, setEquipmentSetFocus] = useState(PET_EQUIPMENT_THEMES[0]);
  ...
  const fashionSlotEntries = Object.entries(PET_FASHION_SLOTS) as [PetFashionSlot, { label: string; symbol: string }][];
  ...
  const rarityOrder: Record<PetRarity, number> = { common: 1, uncommon: 2, rare: 3, epic: 4, legendary: 5 };
  const setCollections = PET_EQUIPMENT_THEMES.map((theme) => {
    const items = profile.inventory.filter((item) => item.theme === theme);
    const equippedItems = items.filter((item) => profile.equipped[item.slot] === item.id);
    return { theme, items, equippedItems, copies: items.reduce((sum, item) => sum + item.count, 0), slots: new Set(items.map((item) => item.slot)).size };
  });
  const focusedSet = setCollections.find((entry) => entry.theme === equipmentSetFocus) ?? setCollections[0];
  const equipmentContext = profile.inventory.filter((item) => (equipmentSlotFilter === "all" || item.slot === equipmentSlotFilter) && (equipmentThemeFilter === "all" || item.theme === equipmentThemeFilter));
  const equipmentFilterCounts = {
    all: equipmentContext.length,
    synthesis: equipmentContext.filter((item) => item.count >= 3 && item.level < PET_EQUIPMENT_MAX_LEVEL).length,
    reforge: equipmentContext.filter((item) => item.count >= 2).length,
    equipped: equipmentContext.filter((item) => profile.equipped[item.slot] === item.id).length,
  };
  const equipmentEffectSort = PET_EQUIPMENT_EFFECT_SORTS.find((option) => option.id === equipmentSort);
  const filteredEquipment = profile.inventory.filter((item) => (equipmentFilter === "all"
    || (equipmentFilter === "synthesis" && item.count >= 3 && item.level < PET_EQUIPMENT_MAX_LEVEL)
    || (equipmentFilter === "reforge" && item.count >= 2)
    || (equipmentFilter === "equipped" && profile.equipped[item.slot] === item.id))
    && (equipmentSlotFilter === "all" || item.slot === equipmentSlotFilter)
    && (equipmentThemeFilter === "all" || item.theme === equipmentThemeFilter))
    .sort((left, right) => {
      const themeDelta = PET_EQUIPMENT_THEMES.indexOf(left.theme) - PET_EQUIPMENT_THEMES.indexOf(right.theme);
      const slotDelta = slotOrder.indexOf(left.slot) - slotOrder.indexOf(right.slot);
      const stable = rarityOrder[right.rarity] - rarityOrder[left.rarity] || right.level - left.level || left.name.localeCompare(right.name, "zh-CN");
      if (equipmentEffectSort) return petEquipmentEffectValue(right, equipmentEffectSort.id) - petEquipmentEffectValue(left, equipmentEffectSort.id) || right.power - left.power || stable;
      if (equipmentSort === "theme_slot") return themeDelta || slotDelta || stable;
      if (equipmentSort === "slot_theme") return slotDelta || themeDelta || stable;
      if (equipmentSort === "power") return right.power - left.power || stable;
      if (equipmentSort === "level") return right.level - left.level || right.power - left.power || stable;
      if (equipmentSort === "rarity") return rarityOrder[right.rarity] - rarityOrder[left.rarity] || right.level - left.level || stable;
      return right.count - left.count || stable;
    });
  const resetEquipmentFilters = () => { setEquipmentFilter("all"); setEquipmentSlotFilter("all"); setEquipmentThemeFilter("all"); };
  ...
  return (
    ...
    {settingsOpen ? <div className="pet-studio-backdrop" role="presentation">
      <section className="pet-studio" role="dialog" aria-modal="true" aria-label="宠物自定义空间" style={{ "--pet-color": petColor } as CSSProperties}>
        <header><div><span>PET STUDIO</span><h2>{profile.name}的自定义空间</h2><p>升级解锁毛色、配饰和 60 件主题时装，打造你的专属标注搭子。</p></div><button type="button" onClick={onToggleSettings} aria-label="关闭宠物自定义空间">×</button></header>
        <div className="pet-studio-body">
          ...
          <div className="pet-studio-editor">
            ...
            {studioSection === "wheel" ? <section className="pet-wheel-panel">
              <header><div><span>LUCKY PRIZE WHEEL</span><h3>进化券幸运大转盘</h3><p>主要用于获得进化券；单次进化券数学期望严格为 1。装备与熔铸零件继续通过摸摸、标注和宠物家园获得。</p></div><b><small>可用次数</small>{profile.wheel_chances}</b></header>
              <div className="pet-wheel-dashboard">
                <div className="pet-wheel-stage">
                  <i className="pet-wheel-pointer" aria-hidden="true" />
                  <div className={`pet-prize-wheel ${wheelSpinning ? "spinning" : ""}`} style={{ "--wheel-rotation": `${wheelRotation}deg`, "--wheel-background": "conic-gradient(#477b5d 0 25%,#2b6c70 25% 50%,#6d5c91 50% 75%,#987331 75% 100%)" } as CSSProperties}>
                ...
                <div className="pet-wheel-info">
                  ...
                  <div className="pet-wheel-odds"><header><strong>奖池概率 · 期望 1 券/次</strong><small>祝福用于十券定向换路线</small></header><div>{profile.wheel_rewards.map((reward) => <span className={`tone-${reward.tone}`} key={reward.id}><i>{reward.icon}</i><b>{reward.label}</b><em>{reward.probability}%</em></span>)}</div></div>
                </div>
              </div>
            ...
            {studioSection === "equipment" ? <section className="pet-collection-panel">
              <header><div><span>EQUIPMENT CODEX</span><h3>装备仓库与套装图鉴</h3></div><div className="pet-equipment-head-actions"><b>{profile.inventory.length} / {profile.equipment_catalog_size}</b><button type="button" onClick={onOpenEquipmentGuide}>查看攻略 ↗</button></div></header>
              <div className="pet-drop-rate-strip"><span><b>5%</b> 摸摸</span><span><b>18%</b> 提交标注</span><span><b>20%</b> Badcase 额外判定</span><small>均为基础概率，装备与技能加成另计</small></div>
              <div className="pet-equipment-rules"><article><b>01</b><div><strong>掉落三选一</strong><span>先看库存、套装与部位，随机词条领取后揭晓</span></div></article><article><b>02</b><div><strong>三件同名强化</strong><span>保留主装备、消耗 2 件材料，按成功率升 1 级</span></div></article><article><b>03</b><div><strong>分解与随机熔铸</strong><span>分解 2 件换 2 枚零件，再随机铸成 1 件装备</span></div></article><article><b>04</b><div><strong>词条洗练</strong><span>消耗 1 件同名材料，等级和强化保底保持不变</span></div></article></div>
              <section className="pet-auto-loadout"><header><div><span>AUTO LOADOUT</span><strong>按场景自动配装</strong><small>会同时比较单件效果、随机词条与套装 2 / 3 / 5 件共鸣；可先在衣柜保存当前搭配。</small></div><b>{Object.keys(profile.equipped).length} / 5 已穿戴</b></header><div>{PET_AUTO_EQUIP_MODES.map((mode) => <button type="button" disabled={busy || !profile.inventory.length} onClick={() => onAutoEquip(mode.id)} key={mode.id}><i>{mode.icon}</i><span><strong>{mode.label}</strong><small>{mode.description}</small></span><em>一键换装</em></button>)}</div></section>
              <section className="pet-forge-workbench"><div><span>RANDOM FORGE</span><strong>随机装备熔炉</strong><p>每分解 1 件装备获得 1 枚零件。消耗 2 枚随机获得 1 件装备并鉴定词条，另有 <b>1%</b> 概率让结果装备额外升 1 级。</p></div><div className="pet-forge-parts"><i>⌘</i><span><small>熔铸零件</small><strong>{profile.equipment_parts}</strong></span><button type="button" disabled={busy || profile.equipment_parts < 2} onClick={onRandomForge}>{busy ? "熔铸中…" : profile.equipment_parts < 2 ? `还差 ${2 - profile.equipment_parts} 枚` : "消耗 2 枚随机熔铸"}</button></div></section>
              ...
              <section className="pet-set-collection"><header><div><span>SET COLLECTION</span><strong>套装收集册</strong><small>点击套装查看已获得的具体装备；再点击“筛选仓库”只看该套装。</small></div><b>{setCollections.filter((entry) => entry.items.length).length} / 12 套已收录</b></header><div className="pet-set-collection-tabs">{setCollections.map((entry) => <button type="button" className={`${equipmentSetFocus === entry.theme ? "active" : ""} ${entry.items.length ? "owned" : ""}`} onClick={() => setEquipmentSetFocus(entry.theme)} key={entry.theme}><span>{entry.theme}</span><b>{entry.items.length}/25</b><i><em style={{ width: `${entry.items.length / 25 * 100}%` }} /></i><small>{entry.slots}/5 部位 · {entry.copies} 件</small></button>)}</div>{focusedSet ? <div className="pet-set-collection-detail"><header><div><span>{focusedSet.theme.toUpperCase()} SET</span><h4>{PET_EQUIPMENT_SET_EFFECTS[focusedSet.theme].name}</h4><p>{PET_EQUIPMENT_SET_EFFECTS[focusedSet.theme].description} · 已收录 {focusedSet.items.length}/25 件图鉴，共 {focusedSet.copies} 件库存。</p></div><button type="button" className={equipmentThemeFilter === focusedSet.theme ? "active" : ""} onClick={() => setEquipmentThemeFilter((current) => current === focusedSet.theme ? "all" : focusedSet.theme)}>{equipmentThemeFilter === focusedSet.theme ? "取消套装筛选" : "筛选仓库中的这套"}</button></header><div>{slotEntries.map(([slot, info]) => { const ownedItems = focusedSet.items.filter((item) => item.slot === slot).sort((a, b) => rarityOrder[b.rarity] - rarityOrder[a.rarity] || b.level - a.level); return <article className={ownedItems.length ? "owned" : ""} key={slot}><header><i><PetEquipmentArt slot={slot} /></i><span><strong>{info.label}</strong><small>{ownedItems.length}/5 已收录</small></span></header>{ownedItems.length ? <div>{ownedItems.map((item) => <button type="button" className={`rarity-${item.rarity}`} onClick={() => { setEquipmentSlotFilter(slot); setEquipmentThemeFilter(focusedSet.theme); }} key={item.id}><span>{item.name}</span><small>{PET_RARITY_LABELS[item.rarity]} · Lv.{item.level} · ×{item.count}</small></button>)}</div> : <p>尚未获得该部位</p>}</article>; })}</div></div> : null}</section>
              <section className="pet-slot-browser"><header><div><span>SLOT BROWSER</span><strong>{equipmentSlotFilter === "all" ? "按部位浏览全部装备" : `${PET_EQUIPMENT_SLOTS[equipmentSlotFilter].label}装备`}</strong><small>点击一个部位，会在下方仓库中显示该部位的全部装备。</small></div><button type="button" className={equipmentSlotFilter === "all" ? "active" : ""} onClick={() => setEquipmentSlotFilter("all")}>全部部位 <b>{profile.inventory.length}</b></button></header><div className="pet-equipped-slots">{slotEntries.map(([slot, info]) => { const itemId = profile.equipped[slot]; const item = profile.inventory.find((owned) => owned.id === itemId); const ownedCount = profile.inventory.filter((owned) => owned.slot === slot).length; return <div className={equipmentSlotFilter === slot ? "active" : ""} key={slot}><button className="pet-slot-select" type="button" onClick={() => setEquipmentSlotFilter((current) => current === slot ? "all" : slot)}><b><PetEquipmentArt slot={slot} /></b><span>{info.label}<small>{item ? `${item.name} · Lv.${item.level}` : "未装备"}</small>{item ? <small className="pet-equipment-effect">{item.effect_label} +{item.effect_value}%{item.affixes.length ? ` · ${item.affixes.length} 条词条` : ""}</small> : null}</span><em>{ownedCount} 件图鉴</em></button>{item ? <button className="pet-slot-unequip" type="button" onClick={() => onEquip(slot, null)}>卸下</button> : null}</div>; })}</div></section>
              {profile.inventory.length ? <><section className="pet-equipment-browser-tools"><div className="pet-equipment-filters" role="group" aria-label="装备筛选">{([['all', '全部'], ['synthesis', '可合成'], ['reforge', '可洗练'], ['equipped', '已装备']] as const).map(([id, label]) => <button type="button" className={equipmentFilter === id ? "active" : ""} onClick={() => setEquipmentFilter(id)} key={id}><span>{label}</span><b>{equipmentFilterCounts[id]}</b></button>)}</div><div className="pet-equipment-sort"><span><b>{filteredEquipment.length}</b> 件结果{equipmentSlotFilter !== "all" ? ` · ${PET_EQUIPMENT_SLOTS[equipmentSlotFilter].label}` : ""}{equipmentThemeFilter !== "all" ? ` · ${equipmentThemeFilter}套装` : ""}</span><label>排序<select value={equipmentSort} onChange={(event) => setEquipmentSort(event.target.value as PetEquipmentSort)} aria-label="装备排序"><option value="theme_slot">套装 → 部位</option><option value="slot_theme">部位 → 套装</option><option value="power">战力从高到低</option>{PET_EQUIPMENT_EFFECT_SORTS.map((option) => <option value={option.id} key={option.id}>{option.label}从高到低</option>)}<option value="level">等级从高到低</option><option value="rarity">稀有度从高到低</option><option value="count">库存数量从多到少</option></select></label>{equipmentFilter !== "all" || equipmentSlotFilter !== "all" || equipmentThemeFilter !== "all" ? <button type="button" onClick={resetEquipmentFilters}>清除筛选</button> : null}</div>{equipmentEffectSort ? <div className="pet-equipment-sort"><span>按单件固定效果与全部词条合计；摸摸、标注、Badcase 含全局掉率加成。套装效果请使用上方一键配装。</span></div> : null}</section>{filteredEquipment.length ? <div className="pet-inventory-grid">{filteredEquipment.map((item) => <article className={`rarity-${item.rarity} ${profile.equipped[item.slot] === item.id ? "active" : ""}`} key={item.id}>
                <div className="pet-inventory-main"><b><PetEquipmentArt slot={item.slot} /></b><span><strong>{item.name}</strong><small>{item.theme}套装 · Lv.{item.level} / {PET_EQUIPMENT_MAX_LEVEL} · 库存 ×{item.count}</small><i className="pet-equipment-level"><em style={{ width: `${item.level * 10}%` }} /></i><small className="pet-equipment-effect">固定：{item.effect_label} +{item.effect_value}%</small>{equipmentEffectSort ? <small className="pet-equipment-effect"><b>{equipmentEffectSort.label}合计：+{petEquipmentEffectValue(item, equipmentEffectSort.id)}{equipmentEffectSort.id === "rarity_boost" ? "" : "%"}</b></small> : null}</span><button type="button" disabled={busy} onClick={() => onEquip(item.slot, profile.equipped[item.slot] === item.id ? null : item.id)}>{profile.equipped[item.slot] === item.id ? "卸下" : "装备"}</button></div>
                <div className="pet-equipment-affixes">{item.affixes.length ? item.affixes.map((affix) => <span className={affix.critical ? "critical" : ""} key={affix.id}>{affix.critical ? "暴击 · " : ""}{affix.label} +{affix.value}{affix.key === "rarity_boost" ? "" : "%"}</span>) : <span className="empty">尚无随机词条，合成或洗练可获得</span>}</div>
                <div className="pet-equipment-actions"><button type="button" disabled={busy || item.count < 3 || item.level >= PET_EQUIPMENT_MAX_LEVEL} onClick={() => onSynthesize(item.id)}><strong>{item.level >= PET_EQUIPMENT_MAX_LEVEL ? "已满级" : `强化至 Lv.${item.level + 1}`}</strong><small>{item.level >= PET_EQUIPMENT_MAX_LEVEL ? "无需继续强化" : `消耗 2 件同名 · ${item.synthesis_success_rate}%${item.synthesis_failures ? `（保底 +${Math.min(30, item.synthesis_failures * 5)}%）` : ""}`}</small></button><button type="button" disabled={busy || item.count < 2} onClick={() => onReforge(item.id)}><strong>洗词条</strong><small>消耗 1 件同名 · 等级保留</small></button><button className="dismantle" type="button" disabled={busy || (profile.equipped[item.slot] === item.id && item.count === 1)} onClick={() => onDismantle(item.id)}><strong>分解 1 件</strong><small>{profile.equipped[item.slot] === item.id && item.count === 1 ? "请先卸下" : "获得 1 枚熔铸零件"}</small></button></div>
              </article>)}</div> : <div className="pet-empty-collection compact"><b>⌕</b><strong>当前组合筛选没有装备</strong><span>切换部位、套装或操作状态，也可以一键清除全部筛选。</span><button type="button" onClick={resetEquipmentFilters}>清除筛选</button></div>}</> : <div className="pet-empty-collection"><b>◇</b><strong>第一件装备正在路上</strong><span>继续摸摸或提交标注，就有机会随机掉落。</span><button type="button" onClick={onOpenEquipmentGuide}>先看看装备攻略</button></div>}
            </section> : null}
            {studioSection === "wardrobe" ? <section className="pet-wardrobe-panel pet-fashion-wardrobe">
              ...
              <section className="pet-fashion-closet"><header><div><span>FREE STYLING</span><strong>自由搭配</strong><small>点击服装立即预览；点击已穿戴服装可卸下。关闭工作室会撤销未保存的试穿。</small></div><div><select value={fashionTheme} onChange={(event) => setFashionTheme(event.target.value)} aria-label="时装主题筛选"><option value="all">全部主题</option>{PET_FASHION_THEMES.map((theme) => <option value={theme.id} key={theme.id}>{theme.name}</option>)}</select><button type="button" onClick={() => { fashionSlotEntries.forEach(([slot]) => onSelectFashion(slot, null)); }}>清空时装</button></div></header><nav><button type="button" className={fashionSlot === "all" ? "active" : ""} onClick={() => setFashionSlot("all")}>全部 <b>{PET_FASHION_CATALOG.length}</b></button>{fashionSlotEntries.map(([slot, info]) => <button type="button" className={fashionSlot === slot ? "active" : ""} onClick={() => setFashionSlot(slot)} key={slot}>{info.symbol} {info.label}<b>{PET_FASHION_CATALOG.filter((item) => item.slot === slot && item.level <= profile.level).length}</b></button>)}</nav><div className="pet-fashion-grid">{visibleFashion.map((item) => { const active = profile.fashion[item.slot] === item.id; const locked = item.level > profile.level; return <button type="button" className={`rarity-${item.rarity} ${active ? "active" : ""} ${locked ? "locked" : ""}`} disabled={busy || locked} onClick={() => onSelectFashion(item.slot, active ? null : item.id)} style={{ "--fashion-primary": item.primary, "--fashion-secondary": item.secondary } as CSSProperties} key={item.id}><i>{<PetFashionArt item={item} icon />}</i><span><strong>{item.name}</strong><small>{item.theme_name} · {item.slot_name}</small></span><em>{locked ? `Lv.${item.level}` : active ? "穿戴中" : "试穿"}</em></button>; })}</div></section>

              <section className="pet-saved-looks"><header><div><span>LOOKBOOK</span><strong>搭配收藏</strong><small>保存时装、毛色、基础配饰与战斗装备，一键恢复完整造型。</small></div><b>{profile.wardrobe_presets.length} / 8</b></header>{profile.wardrobe_presets.length ? <div className="pet-wardrobe-grid">{profile.wardrobe_presets.map((preset, index) => { const color = PET_COLORS.find((item) => item.id === preset.color); const presetAccessory = PET_ACCESSORIES.find((item) => item.id === preset.accessory); const fashionCount = Object.keys(preset.fashion ?? {}).length; const previewProfile = { ...profile, color: preset.color, accessory: preset.accessory, fashion: preset.fashion ?? {}, equipped: preset.equipped }; return <article key={preset.id}><header><span>LOOK {String(index + 1).padStart(2, "0")}</span><button type="button" disabled={busy} onClick={() => onDeleteWardrobe(preset.id)} aria-label={`删除${preset.name}`}>×</button></header><div className="pet-lookbook-preview" style={{ "--pet-color": color?.value ?? PET_COLORS[0].value } as CSSProperties}><PetCreatureVisual profile={previewProfile} accessory={presetAccessory?.symbol} showEquipment={false} /></div><h4>{preset.name}</h4><div className="pet-wardrobe-items">{fashionSlotEntries.map(([slot, info]) => { const item = PET_FASHION_CATALOG.find((candidate) => candidate.id === preset.fashion?.[slot]); return <i className={item ? `rarity-${item.rarity}` : "empty"} title={item?.name ?? `${info.label}未穿戴`} key={slot}>{item ? <PetFashionArt item={item} icon /> : info.symbol}</i>; })}</div><small>{color?.label} · {presetAccessory?.label} · {fashionCount}/5 件时装 · {new Date(preset.created_at).toLocaleDateString("zh-CN")}</small><button className="pet-wardrobe-apply" type="button" disabled={busy} onClick={() => onApplyWardrobe(preset.id)}>套用完整造型</button></article>; })}</div> : <div className="pet-wardrobe-empty"><b>◇</b><strong>搭配收藏还是空的</strong><span>先在上方完成一套穿搭，再为它命名收藏。最多可以保存 8 套完整造型。</span></div>}</section>
            </section> : null}
...
function CandidateAutoScore({ metadata }: { metadata?: JsonObject }) {
  if (!metadata) return null;
  const rawScore = Number(metadata.auto_score);
  const score = Number.isFinite(rawScore) ? rawScore : null;
  const tier = typeof metadata.auto_tier === "string" ? metadata.auto_tier : "";
  const rationale = typeof metadata.auto_score_rationale === "string" ? metadata.auto_score_rationale : "";
  const completion = typeof metadata.auto_completion_rate === "string" ? metadata.auto_completion_rate : "";
  const completionSummary = typeof metadata.auto_completion_summary === "string" ? metadata.auto_completion_summary : "";
  const warning = typeof metadata["⚠️_reliability_warning"] === "string" ? String(metadata["⚠️_reliability_warning"]) : "";
  const reasons = Array.isArray(metadata.reliability_reasons) ? metadata.reliability_reasons.filter((item): item is string => typeof item === "string") : [];
  const subtasks = Array.isArray(metadata.auto_subtasks) ? metadata.auto_subtasks.filter((item): item is string => typeof item === "string") : [];
  if (score === null && !tier && !rationale && !completion && !warning) return null;
  const scoreTone = score === null ? "neutral" : score >= 8 ? "high" : score >= 4 ? "medium" : "low";
  return <section className={`candidate-auto-score score-${scoreTone} ${warning || metadata.high_variance === true ? "unreliable" : ""}`}>
    <header><div><span>AUTO SCORE</span><strong>{score === null ? "—" : score}<small>/ 10</small></strong></div><div>{tier ? <b>{tier}</b> : null}{completion ? <i>完成率 {completion}</i> : null}{warning ? <em>⚠ 可靠性提醒</em> : null}</div></header>
    {rationale ? <p>{rationale}</p> : null}
    {completionSummary || subtasks.length ? <div className="candidate-auto-delivery">{completionSummary ? <strong>{completionSummary}</strong> : null}{subtasks.slice(0, 4).map((item) => <span key={item}>{item}</span>)}</div> : null}
    {warning ? <details><summary>{warning}{typeof metadata.tier_votes === "string" ? ` · ${metadata.tier_votes}` : ""}{typeof metadata.score_gap === "string" ? ` · 分差 ${metadata.score_gap}` : ""}</summary>{reasons.length ? <ul>{reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul> : <p>建议结合模型回复独立判断。</p>}</details> : null}
  </section>;
}
  ...
  return (
    <article className={`candidate-card ${existing?.status === "submitted" ? "submitted" : ""} ${badcase ? "badcase" : ""} ${locked ? "locked" : ""}`}>
      ...
      <section className="candidate-output">
        <CandidateAutoScore metadata={candidate.metadata} />
...
export default function Home() {
  ...
  const petCustomizationSnapshot = useRef<Pick<PetProfile, "name" | "color" | "accessory" | "fashion"> | null>(null);
  ...
  const openPetEquipmentGuide = () => {
    setPetEquipmentGuideOpen(true);
    setMetricsOpen(false);
    setTeamOpen(false);
    setPromptWorkspaceOpen(false);
    setAiOpen(false);
  ...
  useEffect(() => {
    const handleKeys = (event: KeyboardEvent) => {
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "k") {
        if (petHomeOpen || petEquipmentGuideOpen) return;
      ...
      if (event.key === "Escape" && petForgeReveal) {
        setPetForgeReveal(null);
        return;
      }
      if (event.key === "Escape" && petEquipmentGuideOpen) {
        setPetEquipmentGuideOpen(false);
        return;
      }
      ...
      if (event.key === "Escape" && petSettingsOpen) {
        ...
        if (snapshot) {
          const restored = { ...petProfileRef.current, color: snapshot.color, accessory: snapshot.accessory, fashion: { ...snapshot.fashion } };
      ...
      if (editing || event.metaKey || event.ctrlKey || event.altKey || aiOpen || chatOpen || teamOpen || petHomeOpen || petEquipmentGuideOpen || petSettingsOpen || metricsOpen) return;
  ...
  }, [filtered, selectedKey, tab, aiOpen, chatOpen, teamOpen, petHomeOpen, petBattleResult, petForgeReveal, petEquipmentGuideOpen, petSettingsOpen, metricsOpen, selectCase, switchViewTab]);
  ...
  const battleInPetHomestead = async () => {
    ...
    try {
      ...
      else if (result.reward_pending) wakePet("虽然挑战失败，但触发了 10% 幸运掉落！", "happy");
    } catch (error) {
      ...
      if (serverUser) {
        ...
        if (hourlyTouches >= 10) wakePet("本小时摸摸经验已满 2 EXP，陪伴不限量～", "happy");
          ...
          if (earned) {
            ...
            if (rolled.profile !== petProfileRef.current) { applyPetProfile(rolled.profile); wakePet("摸摸掉落了装备，快从 3 件中选 1 件！", "proud"); }
          }
        }
      }
    } catch {
  ...
  const evolveCompanion = async (spend: 1 | 5 | 10, targetPath: PetEvolutionPath = "") => {
    ...
    if (spend === 10 && (!targetPath || targetPath === current.evolution_path || !current.evolution_path)) return;
    if (spend === 5) {
      const message = current.evolution_path && current.evolution_stage > 0
        ? `将消耗 5 张进化券随机更换路线。当前 ${current.evolution_stage} 级路线会清空，新路线从第 1 次进化开始，并补偿 ${current.evolution_stage * 2} 次大转盘；装备和技能保留。继续吗？`
        : "将消耗 5 张进化券完成首次必定成功的进化，路线仍然随机。继续吗？";
      if (!window.confirm(message)) return;
    }
    if (spend === 10) {
      const targetName = PET_EVOLUTION_PATHS[targetPath as Exclude<PetEvolutionPath, "">]?.name;
      const failures = current.targeted_evolution_target === targetPath ? current.targeted_evolution_failures : 0;
      const rate = Math.min(100, 70 + failures * 10 + current.targeted_evolution_blessings * 5);
      if (!window.confirm(`将消耗 10 张进化券，尝试定向更换为「${targetName}」，成功率 ${rate}%。失败时保留当前路线并为同一目标叠加 10% 成功率；成功后补偿 ${current.evolution_stage * 2} 次大转盘。继续吗？`)) return;
    }
    setPetBusy(true);
    try {
      if (serverUser) {
  ...
  const autoEquipPet = async (mode: PetAutoEquipMode) => {
    if (petBusy || !petProfileRef.current.inventory.length) return;
    setPetBusy(true);
    try {
      const modeLabel = PET_AUTO_EQUIP_MODES.find((item) => item.id === mode)?.label ?? "当前场景";
      if (serverUser) {
        const result = await apiRequest<{ profile: PetProfile; score_before: number; score_after: number }>("/api/pet/equipment/auto", { method: "POST", body: JSON.stringify({ mode }) });
        applyPetProfile(result.profile);
        wakePet(`${modeLabel}配装完成：核心指标 ${result.score_before} → ${result.score_after}。`, result.score_after > result.score_before ? "proud" : "happy");
      } else {
        const current = petProfileRef.current;
        const beforeState = petEquipmentState(current.inventory, current.equipped);
        const before = mode === "combat" ? beforeState.stats.total_power * 6 + beforeState.sets.reduce((sum, set) => sum + set.bonuses.length, 0) * 15
          : mode === "evolution" ? beforeState.stats.evolution_bonus
            : mode === "annotation" ? beforeState.stats.all_drop_bonus + beforeState.stats.annotation_drop_bonus
              : mode === "pet" ? beforeState.stats.all_drop_bonus + beforeState.stats.pet_drop_bonus
                : mode === "badcase" ? beforeState.stats.all_drop_bonus + beforeState.stats.badcase_drop_bonus
                  : beforeState.stats.rarity_boost;
        const result = petAutoLoadout(current, mode);
        applyPetProfile({ ...current, equipped: result.equipped });
        wakePet(`${modeLabel}配装完成：核心指标 ${before} → ${result.score[0]}。`, result.score[0] > before ? "proud" : "happy");
      }
    } catch (error) {
      wakePet(error instanceof Error ? error.message : "自动配装失败", "worried");
    } finally {
      setPetBusy(false);
    }
  };

  const savePetWardrobe = async (name: string) => {
    if (petBusy || petProfileRef.current.wardrobe_presets.length >= 8) return;
    setPetBusy(true);
    try {
      const current = petProfileRef.current;
      if (serverUser) {
        await apiRequest<PetProfile>("/api/pet", { method: "PUT", body: JSON.stringify({ name: petDraftName.trim() || current.name, color: current.color, accessory: current.accessory, fashion: current.fashion }) });
        const result = await apiRequest<{ profile: PetProfile }>("/api/pet/wardrobe", { method: "POST", body: JSON.stringify({ action: "save", name: name.trim() }) });
        const saved = applyPetProfile(result.profile);
        petCustomizationSnapshot.current = { name: saved.name, color: saved.color, accessory: saved.accessory, fashion: { ...saved.fashion } };
      } else {
        const preset: PetWardrobePreset = { id: `look-${petRandomInt(1_000_000_000)}-${petRandomInt(1_000_000_000)}`, name: name.trim() || `搭配 ${current.wardrobe_presets.length + 1}`, color: current.color, accessory: current.accessory, fashion: { ...current.fashion }, fashion_saved: true, equipped: { ...current.equipped }, created_at: new Date().toISOString() };
        const saved = applyPetProfile({ ...current, name: petDraftName.trim() || current.name, wardrobe_presets: [preset, ...current.wardrobe_presets] });
        petCustomizationSnapshot.current = { name: saved.name, color: saved.color, accessory: saved.accessory, fashion: { ...saved.fashion } };
      }
      wakePet("当前时装、外观与装备已经收进搭配收藏。", "proud");
    } catch (error) {
      wakePet(error instanceof Error ? error.message : "衣柜保存失败", "worried");
      throw error;
    } finally {
      setPetBusy(false);
    }
  ...
  const applyPetWardrobe = async (presetId: string) => {
    if (petBusy) return;
    setPetBusy(true);
    try {
      let next: PetProfile;
      if (serverUser) {
        const result = await apiRequest<{ profile: PetProfile }>("/api/pet/wardrobe", { method: "POST", body: JSON.stringify({ action: "apply", preset_id: presetId }) });
        next = applyPetProfile(result.profile);
      } else {
        const current = petProfileRef.current;
        const preset = current.wardrobe_presets.find((item) => item.id === presetId);
        if (!preset) throw new Error("衣柜搭配不存在");
        const equipped = Object.fromEntries(Object.entries(preset.equipped).filter(([slot, itemId]) => current.inventory.some((item) => item.id === itemId && item.slot === slot))) as Partial<Record<PetEquipmentSlot, string>>;
        const color = PET_COLORS.some((item) => item.id === preset.color && item.level <= current.level) ? preset.color : current.color;
        const accessory = PET_ACCESSORIES.some((item) => item.id === preset.accessory && item.level <= current.level) ? preset.accessory : current.accessory;
        const fashion = preset.fashion_saved ? Object.fromEntries(Object.entries(preset.fashion ?? {}).filter(([slot, itemId]) => PET_FASHION_CATALOG.some((item) => item.id === itemId && item.slot === slot && item.level <= current.level))) as Partial<Record<PetFashionSlot, string>> : current.fashion;
        next = applyPetProfile({ ...current, color, accessory, fashion, equipped });
      }
      petCustomizationSnapshot.current = { name: next.name, color: next.color, accessory: next.accessory, fashion: { ...next.fashion } };
      wakePet("完整穿搭已套用，缺失装备已自动跳过。", "happy");
    } catch (error) {
      wakePet(error instanceof Error ? error.message : "套用衣柜失败", "worried");
    } finally {
      setPetBusy(false);
    }
  };

  const deletePetWardrobe = async (presetId: string) => {
    const preset = petProfileRef.current.wardrobe_presets.find((item) => item.id === presetId);
    if (petBusy || !preset || !window.confirm(`确定删除衣柜搭配「${preset.name}」吗？不会删除其中的装备。`)) return;
    setPetBusy(true);
    try {
      if (serverUser) {
        const result = await apiRequest<{ profile: PetProfile }>("/api/pet/wardrobe", { method: "POST", body: JSON.stringify({ action: "delete", preset_id: presetId }) });
        applyPetProfile(result.profile);
      } else {
        const current = petProfileRef.current;
        applyPetProfile({ ...current, wardrobe_presets: current.wardrobe_presets.filter((item) => item.id !== presetId) });
      }
      wakePet("衣柜搭配已删除，装备仍在仓库。", "curious");
    } catch (error) {
      wakePet(error instanceof Error ? error.message : "删除衣柜搭配失败", "worried");
    } finally {
      setPetBusy(false);
    }
  };

  const synthesizePetItem = async (itemId: string) => {
    const item = petProfileRef.current.inventory.find((owned) => owned.id === itemId);
    if (petBusy || !item || item.count < 3 || item.level >= PET_EQUIPMENT_MAX_LEVEL) return;
    if (!window.confirm(`合成「${item.name}」将消耗 2 件同名材料，本次成功率 ${item.synthesis_success_rate}%。失败时等级和现有词条保留，但材料仍会消耗并提高下次保底。继续吗？`)) return;
    setPetBusy(true);
    try {
      ...
      } else wakePet(`合成失败，装备与词条已保留；下次成功率提升至 ${nextItem?.synthesis_success_rate ?? result.success_rate + 5}%。`, "worried");
    } catch (error) {
      wakePet(error instanceof Error ? error.message : "装备合成失败", "worried");
    } finally {
      setPetBusy(false);
    }
  };

  const reforgePetItem = async (itemId: string) => {
    const item = petProfileRef.current.inventory.find((owned) => owned.id === itemId);
    if (petBusy || !item || item.count < 2) return;
    if (!window.confirm(`洗练「${item.name}」将消耗 1 件同名材料，并随机替换全部词条；装备等级与合成保底不变。继续吗？`)) return;
    setPetBusy(true);
    try {
      const result = serverUser
        ? await apiRequest<{ profile: PetProfile; affixes: PetEquipmentAffix[] }>("/api/pet/equipment/reforge", { method: "POST", body: JSON.stringify({ item_id: itemId }) })
        : reforgeLocalPetEquipment(petProfileRef.current, itemId);
      applyPetProfile(result.profile);
      const criticalCount = result.affixes.filter((affix) => affix.critical).length;
      wakePet(`洗练完成：获得 ${result.affixes.length} 条随机词条${criticalCount ? `，其中 ${criticalCount} 条暴击` : ""}。`, criticalCount ? "proud" : "curious");
    } catch (error) {
      wakePet(error instanceof Error ? error.message : "洗词条失败", "worried");
    } finally {
      setPetBusy(false);
    }
  };

  const dismantlePetItem = async (itemId: string) => {
    const item = petProfileRef.current.inventory.find((owned) => owned.id === itemId);
    if (!item) return;
    if (!window.confirm(`确定分解「${item.name}」1 件吗？分解后获得 1 枚熔铸零件；当库存归零时，这件装备的等级和词条也会消失。`)) return;
    setPetBusy(true);
    try {
      const result = serverUser
        ? await apiRequest<{ profile: PetProfile; dismantled: Pick<PetEquipment, "id" | "name">; equipment_parts: number }>("/api/pet/equipment/dismantle", { method: "POST", body: JSON.stringify({ item_id: itemId }) })
        : dismantleLocalPetEquipment(petProfileRef.current, itemId);
      applyPetProfile(result.profile);
      wakePet(`已分解 ${item.name}，现在有 ${result.equipment_parts} 枚熔铸零件。`, "curious");
    } catch (error) {
      wakePet(error instanceof Error ? error.message : "装备分解失败", "worried");
    } finally {
      setPetBusy(false);
    }
  };

  const randomForgePetItem = async () => {
    if (!window.confirm("将消耗 2 枚熔铸零件，随机生成 1 件装备和 1 条随机词条；有 1% 概率额外提升结果装备 1 级。继续吗？")) return;
    setPetBusy(true);
    try {
      const result = serverUser
        ? await apiRequest<PetForgeResult>("/api/pet/equipment/random-forge", { method: "POST" })
        : forgeLocalPetEquipment(petProfileRef.current);
      applyPetProfile(result.profile);
      setPetForgeReveal(result);
      const affixValue = `${result.identified_affix.value}${result.identified_affix.key === "rarity_boost" ? "" : "%"}`;
      wakePet(`熔铸得到 ${PET_RARITY_LABELS[result.item.rarity]}「${result.item.name}」${result.level_up ? `，幸运升至 Lv.${result.item.level}` : ""}；词条：${result.identified_affix.label} +${affixValue}。`, result.level_up ? "proud" : "happy");
    } catch (error) {
      wakePet(error instanceof Error ? error.message : "随机熔铸失败", "worried");
    } finally {
      setPetBusy(false);
    }
  };

  const claimPetDrop = async (token: string, itemId: string) => {
    if (petBusy) return;
    setPetBusy(true);
    try {
      const result = serverUser
        ? await apiRequest<{ profile: PetProfile; drop: PetDropEvent; identified_affix: PetEquipmentAffix; affix_added: boolean }>("/api/pet/equipment/claim-drop", { method: "POST", body: JSON.stringify({ token, item_id: itemId }) })
        : claimLocalPetDrop(petProfileRef.current, token, itemId);
      applyPetProfile(result.profile);
      setPetDropReveal({ drop: result.drop, identified_affix: result.identified_affix, affix_added: result.affix_added });
      const affix = result.identified_affix;
      const value = `${affix.value}${affix.key === "rarity_boost" ? "" : "%"}`;
      if (result.affix_added) wakePet(`已领取「${result.drop.name}」！鉴定出${affix.critical ? "暴击词条" : "词条"}：${affix.label} +${value}`, affix.critical || result.drop.rarity === "legendary" ? "proud" : "happy");
      else wakePet(`已领取「${result.drop.name}」作为同名材料；原装备已有 24 条词条，本次鉴定词条未刻录。`, "curious");
    } catch (error) {
      wakePet(error instanceof Error ? error.message : "领取装备失败", "worried");
      if (serverUser) void refreshPetProfile().catch(() => undefined);
    } finally {
      setPetBusy(false);
    }
  };
  ...
  const giftPetTickets = async (userIds: string[], amount: number, note: string) => {
    if (!serverUser || serverUser.role !== "admin") return;
    setPetBusy(true);
    try {
      const result = await apiRequest<{ recipients: ServerUser[]; amount: number; total_amount: number }>("/api/pet/admin/gift-tickets", { method: "POST", body: JSON.stringify({ recipient_user_ids: userIds.map(Number), amount, note }) });
      wakePet(`已向 ${result.recipients.length} 个账号发放 ${result.total_amount} 张进化券。`, "proud");
    } catch (error) {
  ...
  const completePetWheelSpin = (result: PetWheelSpinResult) => {
    applyPetProfile(result.profile);
    wakePet(`幸运转盘抽中「${result.reward.detail}」！`, result.reward.reward_id === "ticket_5" ? "proud" : "happy");
  };
  ...
  const togglePetStudio = () => {
    ...
    const current = petProfileRef.current;
    petCustomizationSnapshot.current = { name: current.name, color: current.color, accessory: current.accessory, fashion: { ...current.fashion } };
  ...
  const savePetCustomization = async () => {
    ...
    try {
      ...
      if (serverUser) {
        const saved = await apiRequest<PetProfile>("/api/pet", { method: "PUT", body: JSON.stringify({ name, color: draft.color, accessory: draft.accessory, fashion: draft.fashion }) });
  ...
  const previewPetFashion = (slot: PetFashionSlot, itemId: string | null) => {
    const current = petProfileRef.current;
    const fashion = { ...current.fashion };
    if (itemId) {
      const item = PET_FASHION_CATALOG.find((candidate) => candidate.id === itemId && candidate.slot === slot && candidate.level <= current.level);
      if (!item) return;
      fashion[slot] = item.id;
    } else {
      delete fashion[slot];
    }
    const next = { ...current, fashion };
    petProfileRef.current = next;
    setPetProfile(next);
  };
  ...
  const saveCandidateAnnotation = async (candidate: CandidateOutput, value: { scores: Record<string, number>; badcase: boolean; badcaseTags: string[]; note: string }, status: "draft" | "submitted", silent = false): Promise<boolean> => {
    ...
    if (!isTeamSave && status === "submitted") {
      ...
      if (earned) {
        ...
        if (rolled.profile !== petProfileRef.current) { applyPetProfile(rolled.profile); wakePet("标注掉落了装备，快从 3 件中选 1 件！", "proud"); }
      }
    }
