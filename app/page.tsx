...
function PetCreatureVisual({ profile, accessory, showEquipment = true, showFashion = true }: { profile: PetProfile; accessory?: string; showEquipment?: boolean; showFashion?: boolean }) {
  ...
  return <span className={`pet-creature ${illustrated ? "pet-illustrated" : ""} evolution-${path || "base"} evolution-stage-${profile.evolution_stage} evolution-variant-${profile.evolution_variant}`} aria-hidden="true">
    ...
    {showEquipment ? equippedItems.map((item) => <span className={`pet-equipment pet-equipment-${item.slot} rarity-${item.rarity}`} key={item.slot}>{item.symbol}</span>) : null}
  </span>;
}
  ...
  return <div className="pet-drop-backdrop" role="presentation">
    <section className="pet-drop-modal" role="dialog" aria-modal="true" aria-label="装备掉落三选一">
      ...
      <div className="pet-drop-choice-grid">{pending.choices.map((choice, index) => <article className={`rarity-${choice.rarity}`} key={choice.id}>
        ...
        <div className="pet-drop-choice-title"><i>{choice.symbol}</i><div><em>{PET_RARITY_LABELS[choice.rarity]} · {choice.slot_name}</em><strong>{choice.name}</strong><small>{choice.theme}主题 · 固定：{choice.effect_label} +{choice.effect_value}%</small></div></div>
...
function PetDropRevealModal({ result, queueSize, onClose }: { result: PetDropReveal | null; queueSize: number; onClose: () => void }) {
  ...
  return <div className="pet-drop-backdrop pet-reveal-backdrop" role="presentation">
    <section className={`pet-drop-reveal rarity-${drop.rarity}`} role="dialog" aria-modal="true" aria-label="装备词条鉴定结果">
      ...
      <div className="pet-reveal-item"><i>{drop.symbol}</i><div><small>{drop.theme}主题 · {drop.slot_name}</small><h2>{drop.name}</h2><p>固定效果 · {drop.effect_label} +{drop.effect_value}%</p></div></div>
...
function PetForgeRevealModal({ result, onClose }: { result: PetForgeResult | null; onClose: () => void }) {
  ...
  return <div className="pet-drop-backdrop pet-reveal-backdrop pet-forge-reveal-backdrop" role="presentation">
    <section className={`pet-drop-reveal pet-forge-reveal rarity-${item.rarity}`} role="dialog" aria-modal="true" aria-label="随机熔铸结果">
      ...
      <div className="pet-reveal-item"><i>{item.symbol}</i><div><small>{PET_RARITY_LABELS[item.rarity]} · {item.theme}套装 · {item.slot_name}</small><h2>{item.name}</h2><p>固定效果 · {item.effect_label} +{item.effect_value}%</p></div></div>
...
function PetEquipmentGuide({ open, profile, onClose }: { open: boolean; profile: PetProfile; onClose: () => void }) {
  ...
  return <section className="pet-equipment-guide-page" aria-label="装备攻略">
    ...
    <div className="pet-guide-scroll">
      ...
      <section className="pet-guide-section"><header><span>03 · SLOT MAP</span><h3>五个部位的固定职责</h3><p>固定效果只由部位决定，稀有度与等级提高数值。</p></header><div className="pet-guide-slot-map">{slotEntries.map(([slot, info], index) => <article key={slot}><i>{info.symbol}</i><span><strong>{info.label}</strong><small>{PET_EQUIPMENT_EFFECTS[slot].label}</small></span><b>部位 {String(index + 1).padStart(2, "0")}</b></article>)}</div></section>
